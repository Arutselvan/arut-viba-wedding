 ...
 * EnvelopeCover — a full-screen digital wedding invitation envelope.
 * The wax seal uses the actual Aruvi SVG path (no image generation) for pixel-perfect accuracy.
 * Animation sequence on click:
 *   1. Flap unfolds upward (3D rotateX, 0.7s)
 *   2. Wax seal cracks: scale up + fade out (0.4s, delayed 0.3s)
 *   3. Envelope slides down off-screen (0.8s, delayed 0.8s)
 *   4. Site content fades up (0.6s, delayed 1.2s)
 */
...
// The exact path data from Aruvi.svg (transform applied to normalise viewBox)
...
export function EnvelopeCover({ onOpen }: EnvelopeCoverProps) {
  ...
  const handleOpen = () => {
    ...
    setPhase("opening");
    // After full animation, notify parent
    setTimeout(() => {
      setPhase("gone");
      onOpen();
    }, 1800);
  ...
  return (
    <div
      ...
      role="button"
      tabIndex={0}
      onKeyDown={(e) => e.key === "Enter" || e.key === " " ? handleOpen() : undefined}
    >
      {/* ── Envelope body (slides down on open) ── */}
      <div
        className="absolute inset-0"
        style={{
          transition: isOpening ? "transform 0.9s cubic-bezier(0.4, 0, 0.2, 1) 0.9s" : "none",
          transform: isOpening ? "translateY(110%)" : "translateY(0)",
        }}
      >
        {/* Background: hero watercolour */}
        <img
          src={HERO_ART}
          alt=""
        ...
        {/* Parchment overlay for envelope feel */}
        <div
          className="absolute inset-0"
          style={{
            background: "linear-gradient(160deg, rgba(250,246,240,0.82) 0%, rgba(250,246,240,0.65) 40%, rgba(250,246,240,0.45) 100%)",
          }}
        />

        {/* Envelope bottom-triangle fold lines (decorative) */}
        <svg
          ...
          style={{ height: "30%" }}
        >
          <polygon points="0,30 50,0 100,30" fill="rgba(232,217,192,0.45)" />
          <line x1="0" y1="30" x2="50" y2="0" stroke="rgba(212,168,83,0.25)" strokeWidth="0.3" />
          <line x1="100" y1="30" x2="50" y2="0" stroke="rgba(212,168,83,0.25)" strokeWidth="0.3" />
        </svg>

        {/* Envelope left/right side fold lines */}
        <svg
          className="absolute inset-0 w-full h-full"
          viewBox="0 0 100 100"
          preserveAspectRatio="none"
        >
          <line x1="0" y1="100" x2="50" y2="50" stroke="rgba(212,168,83,0.2)" strokeWidth="0.2" />
          <line x1="100" y1="100" x2="50" y2="50" stroke="rgba(212,168,83,0.2)" strokeWidth="0.2" />
        </svg>
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center gap-4 px-6">
          ...
          <img
            ...
            style={{ filter: "drop-shadow(0 2px 8px rgba(28,20,16,0.18))" }}
            draggable={false}
          />

          <div className="text-center space-y-1">
            <p className="font-display text-xs md:text-sm tracking-[0.45em] uppercase text-ink/50">Together with their families</p>
            <h1 className="font-display text-4xl md:text-6xl lg:text-7xl font-light text-ink leading-none">
              Arut <em className="text-saffron not-italic">&amp;</em> Viba
            </h1>
            <p className="font-display text-sm md:text-base italic text-ink/55">request the honour of your presence</p>
          </div>

          <div className="flex flex-col items-center gap-0.5">
            <p className="font-body text-[10px] md:text-xs tracking-[0.3em] uppercase text-ink/50">23rd – 24th January 2027</p>
            <p className="font-body text-[10px] md:text-xs tracking-[0.18em] uppercase text-ink/40">MGM Beach Resorts · Chennai, Tamil Nadu</p>
          </div>

          {/* Wax seal — CSS circle + actual Aruvi SVG path in gold */}
          <div
            className="relative mt-2"
            style={{
              transition: isOpening ? "transform 0.5s ease 0.25s, opacity 0.5s ease 0.25s" : "none",
              transform: isOpening ? "scale(1.4)" : "scale(1)",
          ...
          >
            ...
            <div
              style={{
                ...
                boxShadow: "0 4px 24px rgba(107,15,26,0.45), inset 0 1px 3px rgba(255,200,100,0.15), 0 0 0 2px rgba(212,168,83,0.35)",
                display: "flex",
                alignItems: "center",
            ...
            >
              ...
              />
              {/* Aruvi SVG path — gold tint */}
              <svg
                viewBox="73.331596 247.26931 890.60754 446.49603"
                style={{ width: 88, height: 44, fill: "#D4A853", filter: "drop-shadow(0 1px 2px rgba(0,0,0,0.4))" }}
              >
                <path d={ARUVI_PATH} />
              </svg>
            </div>
          ...
          {/* Tap prompt */}
          <p
            className="font-body text-[10px] tracking-[0.35em] uppercase text-ink/35 mt-1"
            style={{
      ...
      <div
        ...
        style={{ height: "55%", perspective: "800px", perspectiveOrigin: "50% 0%" }}
      >
        <div
          style={{
            width: "100%",
            ...
            transition: isOpening ? "transform 0.75s cubic-bezier(0.4, 0, 0.2, 1) 0.05s" : "none",
            transform: isOpening ? "rotateX(-180deg)" : "rotateX(0deg)",
            backfaceVisibility: "hidden",
          }}
        >
          {/* Flap background — same hero image */}
          <img
            src={HERO_ART}
            alt=""
            ...
            style={{
              ...
              height: "200%", // show top portion of image
            }}
            draggable={false}
          />
          <div
            className="absolute inset-0"
            style={{
              background: "linear-gradient(160deg, rgba(250,246,240,0.88) 0%, rgba(250,246,240,0.7) 60%, rgba(250,246,240,0.5) 100%)",
            }}
          />
          {/* Flap triangle clip */}
          <div
            ...
            style={{
              ...
              background: "rgba(232,217,192,0.3)",
          ...
          <svg
            ...
            style={{ height: 2 }}
          >
            <line x1="0" y1="1" x2="100" y2="1" stroke="rgba(212,168,83,0.35)" strokeWidth="1" />
          </svg>
        </div>
      </div>

      {/* ── Shimmer / particle effect on open ── */}
      {isOpening && (
        <div className="absolute inset-0 pointer-events-none" style={{ zIndex: 10 }}>
          {[...Array(12)].map((_, i) => (
            <div
              ...
              style={{
                position: "absolute",
                left: `${40 + Math.sin(i * 30) * 15}%`,
                top: `${45 + Math.cos(i * 25) * 10}%`,
                width: 4 + (i % 3) * 2,
                height: 4 + (i % 3) * 2,
                borderRadius: "50%",
                background: "#D4A853",
                opacity: 0,
                animation: `sparkle 0.8s ease-out ${0.2 + i * 0.06}s forwards`,
              }}
            />
          ))}
        </div>
