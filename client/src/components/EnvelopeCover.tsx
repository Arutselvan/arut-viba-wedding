 ...
 *
 * Architecture:
 * - Envelope body = full-screen dark forest green rectangle
 * - Four triangular panels (bottom, left, right, top) formed with clip-path
 * - Top flap: rotates on transformOrigin "top center" with rotateX(-180deg)
 *   backface-visibility: hidden ensures the back face is invisible when folded
 *   opacity also fades to 0 during rotation for a clean reveal
 * - Wax seal fades + scales on open
 * - After flap opens, entire envelope fades out gracefully (no hard slide)
 */
...
export default function EnvelopeCover({ onOpen }: { onOpen: () => void }) {
  ...
  useEffect(() => {
    ...
    if (!flap || !seal || !envelope || !body) return;

    // Step 1 (0ms): Seal cracks and fades
    seal.style.transition = "transform 0.5s cubic-bezier(0.4,0,0.6,1), opacity 0.5s ease";
    seal.style.transform = "translate(-50%, -50%) scale(1.4)";
    seal.style.opacity = "0";

    // Step 2 (300ms): Flap folds backward — rotateX(-180deg) folds it up and away
    // backface-visibility: hidden means the back face is invisible once past 90deg
    // We also fade opacity so there's no green rectangle visible at all
    setTimeout(() => {
      flap.style.transition = "transform 0.9s cubic-bezier(0.4, 0, 0.2, 1), opacity 0.9s ease";
      flap.style.transform = "rotateX(-180deg)";
      flap.style.opacity = "0";
    }, 300);

    // Step 3 (1300ms): Entire envelope fades out smoothly — no hard slide
    setTimeout(() => {
      body.style.transition = "opacity 0.8s ease";
      body.style.opacity = "0";
    }, 1300);

    // Step 4 (1500ms): Reveal website
    setTimeout(() => {
      onOpen();
    }, 1500);

    // Step 5 (2200ms): Remove from DOM
    setTimeout(() => {
      setPhase("done");
    }, 2200);
  }, [phase, onOpen]);
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Top flap — folds backward, backface-visibility hidden so green disappears past 90deg */}
        <div
          ref={flapRef}
          style={{
            position: "absolute",
        ...
        {/* Wax seal — centred at the fold line */}
        <div
          ref={sealRef}
          style={{
            position: "absolute",
